...
func resourceAwsGreengrassv2ComponentRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[WARN] Greengrassv2 component %q not found, removing from state", getComponentOutput)

	d.Set("arn", d.Id())
	// d.Set("inline_recipe", *(*string)(unsafe.Pointer(&getComponentOutput.Recipe)))
	// d.Set("lambda_function", *(*string)(unsafe.Pointer(&getComponentOutput.Recipe)))

	// lambdaFunction := flattenLambdaImageConfig(getComponentOutput.Recipe)
	// log.Printf("[INFO] Setting Lambda %s Image config %#v from API", d.Id(), imageConfig)
	// if err := d.Set("lambda_function", imageConfig); err != nil {
	// 	return fmt.Errorf("error setting image config for Lambda Function: %w", err)
	// }

	// if err := d.Set("component_dependencies", deadLetterConfig); err != nil {
	...
	log.Printf("[WARN] Greengrassv2 component %q not found, removing from state", describeComponentOutput)
